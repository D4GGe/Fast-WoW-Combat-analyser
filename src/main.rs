...
fn main() {
    ...
    // 2. Setup cross-thread shutdown signal
    let shutdown = Arc::new(Notify::new());
    let shutdown_for_server = shutdown.clone();
    let shutdown_for_api = shutdown.clone();

    // 3. Start HTTP server in background thread (with its own tokio runtime)
    ...
    // 5. Run the native GUI window (blocks until closed or Stop pressed)
    gui::run(shutdown.clone(), &log_dir_display, PORT);

    // 6. Wait for server thread to finish gracefully
...
fn resolve_log_dir() -> PathBuf {
    ...
    // Always show folder picker — pre-select default path if it exists
    let default = PathBuf::from(DEFAULT_LOG_DIR);
    let start_dir = if default.exists() {
        DEFAULT_LOG_DIR
    } else {
        r"C:\"
    };
